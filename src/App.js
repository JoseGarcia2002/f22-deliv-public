...
export function App() {
  ...
  // Reference video for snapshot functionality https://www.youtube.com/watch?v=ig91zc-ERSE
	...
	// displayEntries are the entries rendered on the screen: initially all the entries
	const [entries, setEntries] = useState([]);
	const [displayEntries, setDisplay] = useState([]);

	useEffect(() => {
		// ! Database query filters entries for current user. DO NOT CHANGE, editing this query may cause it to fail.
		const q = currentUser?.uid ? query(collection(db, "entries"), where("userid", "==", currentUser.uid), limit(25)) : collection(db, "entries");
		...
		onSnapshot(q, (snapshot) => {
			...
			let initialEntries = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }))
			setEntries(initialEntries);
			setDisplay(initialEntries);
		})
	}, [currentUser]);

	// Initialize state and setters for the AlertMessage component
		...
		if (isSignedIn) {
			return (
				<Grid container spacing={3}>
					{/* Set up the context provider to pass Alert state down the component tree */}
					<AlertContext.Provider value={{setAlert, setAlertMessage, setSuccess}}>
						<Grid item xs={12}>
							<Stack direction="row" spacing={1}>
								<Search displayEntries={entries} setDisplay={setDisplay}/>
								<FilterMenu displayEntries={entries} setDisplay={setDisplay}></FilterMenu>
								<EntryModal entry={emptyEntry} type="add" user={currentUser} />
							</Stack>
						</Grid>
						<Grid item xs={12}>
							<AlertMessage alertOpen={alertOpen} setAlert={setAlert} message={alertMessage} status={alertSuccess}/>
							<EntryTable entries={displayEntries} user={currentUser} />
						</Grid>
	...
	return (
		<ThemeProvider theme={mdTheme}>
			...
			>
				...
				{mainContent()}
				<Lights darkMode={darkMode} setDark={setDark}/>
				</Container>
			</Box>
			</Box>
		</ThemeProvider>
